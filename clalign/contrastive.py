...
class CLDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        tms = torch.as_tensor([x[2] for x in batch], dtype=torch.float32)
        offsets = torch.as_tensor([0] + [len(x[0][1]) for x in batch]).cumsum(dim=0)
        return inputs, torch.cat(aln), tms, offsets
...
def train(tokenizer: PreTrainedTokenizer, model: PreTrainedModel, model_path: Path | str, train_data: Path | str,
    ...
    for epoch_idx in range(num_epochs):
        for inputs, aln, tms, offsets in tqdm(train_loader, desc=f'Epoch {epoch_idx}', leave=False,
