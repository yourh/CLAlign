...
def load_structure(fpath, chain=None):
    ...
    elif fpath.endswith('pdb') or fpath.endswith('ent'):
        with open(fpath) as fin:
...
def extract_coords_from_structure(structure: biotite.structure.AtomArray):
    ...
    # coords_list, res_list = [], []
    # for c_, r_ in zip(coords, residue_identities):
    #     if not (~np.isnan(c_)).all():
    #         continue
    #     coords_list.append(c_)
    #     res_list.append(ProteinSequence.convert_letter_3to1(r_) if r_ in ProteinSequence._dict_3to1 else 'X')
    # seq = ''.join([ProteinSequence.convert_letter_3to1(r) for r in residue_identities])
    seq = ''.join([ProteinSequence._dict_3to1.get(r, 'X') for r in residue_identities])
    return coords, seq
    # return coords
    # return coords, ''
    # return np.asarray(coords_list), ''.join(res_list)
...
class CoordBatchConverter(BatchConverter):
    def __call__(self, raw_batch: Sequence[Tuple[Sequence, str]], device=None):
        ...
        # return {'tokens': tokens, 'coords': coords, 'confidence': confidence, 'encoder_padding_mask': padding_mask}
        return {'coords': coords, 'confidence': confidence, 'encoder_padding_mask': padding_mask}
